                ...
                <div class="slider-wrapper" ref="sliderWrapper">
                    <slider>
                        <div v-for="item in sliderRecommends" v-bind:key="item.id" v-on:click="selectItem(item)">
                            <a>
                                <img v-bind:src="item.cover" height="280" alt="">
                            </a>
                        </div>
                    </slider>
                </div>
...
<script>
	...
	export default {
		...
		beforeCreate() {
			this.$http.get("/recommendData").then((data) => {
				this.sliderRecommends = data.data[1].categoryList;//第一个歌单列表作为轮播图的部分
		...
		methods:{
			...
			selectItem(item){
				this.$http.get(`/recommendDetail/${item.id}`).then((data)=>{
					// eslint-disable-next-line no-console
                    console.log(data.data[0])
					if (data.data[0]) {
						this.$router.push(`/recommend/${item.id}`);//设置路由
						this.setDisc(data.data[0]);//对应推荐歌单的歌曲
                    }
				})
